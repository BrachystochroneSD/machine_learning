...
const int POP_NUMBER=100;
const double MUT_RATE=10;
const int POW_SCORE=8;
...
int main(void){
  ...
  for( int i=0; i < NUM_CITIES; i++ ){
    city tmp={(double)rand()/RAND_MAX*100,(double)rand()/RAND_MAX*100};
    CITIES[i]=tmp;
  }

  // create a population of randomly shuffled cities
  city **population=malloc(NUM_CITIES*POP_NUMBER*sizeof(city));
...
void remove_city(city c, city* list_cities, size_t n){
  ...
  if (new_size>0){
    ...
    for(size_t i=0; i<new_size+1; i++){
      int testx=list_cities[i].x;
      int testy=list_cities[i].y;
...
void cross_and_mut(city **pop){
  ...
  city **temp_pop=malloc(NUM_CITIES*POP_NUMBER*sizeof(city));
  for(size_t i=0; i<POP_NUMBER;i++){
    size_t i1 = pick();
